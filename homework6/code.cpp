...
public:
    ...
    */
    ...
    void BilateralFilter(int len, double sigma_dis, double sigma_Y) {
        if (len <= 0 || sigma_dis <= 0 || sigma_Y <= 0) return;
        else len = ((len + 1) / 2 * 2 - 1);
...
void solve() {
    ...
    int n1, len, sigma_dis, sigme_Y;
    cout << "Please input the number of times to use mean fliter: " << endl;
    cin >> n1;
    cout << "Please input the length of mean fliter core (should be an odd number larger than 0, or will be changed automatically): " << endl;
    cin >> len;
    cout << "Please input the standard deviation of the distance (should be a positive number larger than 0, or will return directly): " << endl;
    cin >> sigma_dis;
    cout << "Please input the standard deviation of the Y (should be a positive number larger than 0, or will return directly): " << endl;
    cin >> sigme_Y;

    bmp.RGB2YUV();
    for (int i = 0; i < n1; i ++) bmp.BilateralFilter(len, sigma_dis, sigme_Y);
    bmp.YUV2RGB();
    bmp.WriteNewRGB("./results/bilateral_fliter.bmp");
    bmp.WriteNewgrey("./results/bilateral_fliter_grey.bmp");
