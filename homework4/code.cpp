...
void solve() {
    ...
    // translation
    int bh, bw;
    cout << "please input the shifting of Y and X(int, can be negative)" << endl;
    cin >> bh >> bw;
    bmp.PrepareTransform(); // prepare for transform
    bmp.Translation(bh, bw);
    bmp.WriteTransform("./results/translation.bmp");

    // rotation
    double deg;
    cout << "please input the rotating of deg(double, can be negative)" << endl;
    cin >> deg;
    bmp.PrepareTransform();
    bmp.Rotation(deg);
    bmp.WriteTransform("./results/rotation.bmp");

    // scale
    double ratioH, ratioW;
    cout << "please input the ratio of changing height and width(double, should be positive)" << endl;
    cin >> ratioH >> ratioW;
    bmp.PrepareTransform();
    bmp.Scale(ratioH, ratioW);
    bmp.WriteTransform("./results/scale.bmp");

    // shear
    double dH, dW;
    cout << "please input the shear parameter of changing height and width(double, can be negative)" << endl;
    cin >> dH >> dW;
    bmp.PrepareTransform();
    bmp.Shear(dH, dW);
    bmp.WriteTransform("./results/shear.bmp");

    // mirror
    int axis;
    cout << "please input the edge to mirror(0 means horizontal, 1 means vertical)" << endl;
    cin >> axis;
    bmp.PrepareTransform();
    bmp.Mirror(axis);
    bmp.WriteTransform("./results/mirror.bmp");
...
//     bmp.Shear(2, 0);
//     bmp.Shear(0, 2);
//     bmp.WriteTransform("./test.bmp");
//     bmp.PrepareTransform();
//     bmp.Shear(2, 2);
//     bmp.WriteTransform("./test2.bmp");
// }
